'''
Adapted from slimmable neural network codebase
https://raw.githubusercontent.com/JiahuiYu/slimmable_networks
'''
...
class SwitchableBatchNorm2d(nn.Module):
    ...
    def forward(self, input):
        return self.bn[self.curr_idx](input)


class Conv2dWSE(nn.Conv2d):
    def __init__(self, in_channels_list, out_channels_list,
                 kernel_size, stride=1, padding=0, dilation=1,
                 groups_list=[1], bias=True):
        super(Conv2dWSE, self).__init__(
    ...
    def forward(self, input):
        self.in_channels = self.in_channels_list[self.curr_idx]
        self.out_channels = self.out_channels_list[self.curr_idx]
        self.groups = self.groups_list[self.curr_idx]
