...
prune_max_channelwise(const scalar_t *__restrict__ x, scalar_t *__restrict__ y,
                      const int32_t prune_type, const int32_t nonzero,
                      const int32_t group_size, const int32_t B,
...
prune_kernel(const scalar_t *__restrict__ x, scalar_t *__restrict__ y,
  ...
  if (prune_type == PRUNE_MAX && prune_dim == 0) {
    prune_max_batchwise<scalar_t>(x, y, prune_type, nonzero, group_size, B, C,
                                  W, H);
  } else if (prune_type == PRUNE_MAX && prune_dim == 1) {
    prune_max_channelwise<scalar_t>(x, y, prune_type, nonzero, group_size, B, C,
                                    W, H);
  }
}
} // namespace
